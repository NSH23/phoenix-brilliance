...
/** Cloudinary URLs WhatsApp can fetch as native document media (not chat links). */
function whatsappDocumentUrlCandidates(docUrl) {
  ...
  if (base.indexOf('/raw/upload/') !== -1) {
    var imagePath = base.replace('/raw/upload/', '/image/upload/');
    out.push(/\.pdf$/i.test(imagePath) ? imagePath + qs : imagePath + '.pdf' + qs);
    out.push(imagePath + qs);
  }

  if (base.indexOf('/image/upload/') !== -1) {
    out.push(/\.pdf$/i.test(base) ? base + qs : base + '.pdf' + qs);
    out.push(base + qs);
  }
...
async function pickReachableDocumentUrl(docUrl) {
  var candidates = whatsappDocumentUrlCandidates(docUrl);
  if (!candidates.length) return docUrl;
  for (var i = 0; i < candidates.length; i++) {
    var url = candidates[i];
    try {
      var head = await axios.head(url, { timeout: 15000, maxRedirects: 5 });
      if (head.status >= 200 && head.status < 400) {
        console.log('Document URL reachable:', url.substring(0, 100), head.headers['content-type'] || '');
        return url;
      }
    } catch (e) {
      console.log('Document URL not reachable:', url.substring(0, 80));
    }
  }
  return candidates[0];
}
...
async function sendDocument(phone, docUrl, filename, caption, logMeta, strict) {
  ...
  var link = await pickReachableDocumentUrl(docUrl);
  var docPayload = { link: link, filename: fname };
  if (caption && String(caption).trim()) docPayload.caption = String(caption).trim();

  console.log('Sending document to', phone, ':', link.substring(0, 120));
  try {
...
app.post('/admin-send-media', requireAdmin, async function(req, res) {
  try {
    ...
    if (mediaType === 'image') {
      ...
      await sendDocument(phone, url, filename, caption, logMeta, true);
    } else {
